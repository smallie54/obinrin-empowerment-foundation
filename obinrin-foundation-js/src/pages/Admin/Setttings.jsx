...
export default function Settings() {
  ...
  useEffect(() => {
    if (admin?.role === "superadmin") {
      api.get("/admin/auth/admins").then((res) => setAdmins(res.data)).catch(() => {});
    }
  }, [admin]);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {admin?.role === "superadmin" && (
        <div className="bg-white rounded-2xl border border-charcoal/10 p-6">
          <h3 className="font-heading font-bold text-charcoal mb-4">Admin Accounts</h3>
          ...
          <p className="text-xs text-charcoal/40 mt-4">
            New admin accounts are created via the API (POST /api/admin/auth/create) —
            no UI form for this yet.
          </p>
        </div>
      )}
    </div>
